...
switch SetThreshold
    ...
    case 1
        if ch==0 || ch==2 &&~isempty(data_ch0)
            ...
            [data_th_ch0]=Threshold_ONI(data_ch0,savefilename,ThTable,prompt,1); % need to add shift parameter
        ...
        if ch==1 || ch==2 &&~isempty(data_ch1)
            ...
            [data_th_ch1]=Threshold_ONI(data_ch1,savefilename,ThTable,prompt,1); % need to add shift parameter
...
if do_driftcorrect==1
    ...
    switch SetThreshold
        ...
        case 1
            savefilename=[commonname];
            [data_th_ch1_shift]=Threshold_ONI(data_ch1_shift,savefilename,ThTable,prompt,0); % need to add shift parameter
...
%% Tip position
   ...
   [~,InBG,InEdgeBW] = segmentation(imadd(ch1,ch2),BW3);
    ...
    MVlengthlimit_min = (MVlengthlimit_min_nm/resolvenm)+cellbodywidthpix;%2;%( pixel)
    MVlengthlimit_max = (MVlengthlimit_max_nm/resolvenm)+cellbodywidthpix;%100;
...
%---- write image
    ...
    set(gca,'YTick',[]);
    set(gca,'XTick',[]);
    set(gca,'box','on'); %you take off the box ticks around
    set(gca,'YColor','k')
    set(gca,'XColor','k')
    pause (1)
    F = getframe;
    ...
    imwrite(PixSizeimage,['STORMpiximg_',savefilename,'.tif'],'Compression','none')%,'TileSize',[200 200])%,...
    end
close all
end
...
if ccresultsave==1
    ...
    switch shiftfindoption
        case 1
            title(['Weighted Centroid '])%[ xshift (pix): ', num2str(xshift_nm,'%0.3f'),' yshift:', num2str(yshift, '%0.3f'),' ]'])
        ...
        case 2
            title([' 2D Gaussian Fit'])%[ xshift: ',num2str(xshift,'%0.3f'),' yshift:', num2str(yshift, '%0.3f'),' ]'])
...
imwrite(W,['Rend',num2str(RendSize),'_',savefilename,'.tif'],'Compression','none')%,'TileSize',[200 200])%,...
end
...
switch datatype
    ...
    case '3D'
        ...
        subplot(2,4,5);
        histogram(data(:,13),'edgecolor','none');%,25);
        AvePho=mean(data(:,13));
        medPho=median(data(:,13));
        minPho=min(data(:,13));
        maxPho=max(data(:,13));
        ...
        subplot(2,4,6);
        histogram(data(:,14),'edgecolor','none');%,25);
        meanbg=mean(data(:,14));
        stdbg=std(data(:,14));
        title(['   mean= ',num2str(meanbg,'%.0f'),' std=', num2str(stdbg,'%.0f')])
        xlabel('background','FontSize',14');
...
saveas(gcf,['Paircorr_3D_' ,SaveName]);%,'jpg');    
...
in_radii = rad_bins(1:end-1); % in-radius for each ring
out_radii = rad_bins(2:end); %out-radius for each ring
mean_r = (in_radii+out_radii)/2; %center point of ring 
 ...
 if N1_ws < 1 
    ...
    totalDensity = (N2_ws)/(ws(1)*ws(2)*ws(3));
     ...
     for p=1:N1_ws
         ...
         [result] = (hist(distance,rad_bins(1:end-1))); % !!!!!!!! histc to hist
