...
%[c12, radii,err]= pair_corr_cross_2D(data1, data2, myrange, dr, rMax, pixelsize);
rMax=tipws/2;
pw=tipws/2;
...
%    x_r= (max_x-min_x).*rand(length(N1_ws.x))+min_x;
%    rand_data1.y=x_r;
%    
%    y_r= (max_y-min_y).*rand(length(N1_ws.y))+min_y; 
%    rand_data1.y=y_r;
%    
%    z_r= (max_z-min_z).*rand(length(N1_ws.z))+min_z;
%     rand_data1.z=z_r;
