...
for fr=1:5
    ...
    MVlengthlimit_min = (MVlengthlimit_min_nm*Exfactor/pixelsize)+cellbodywidthpix;%2(pixel) % mimumun length limit in pixel
    MVlengthlimit_max = (MVlengthlimit_max_nm*Exfactor/pixelsize)+cellbodywidthpix;%100;     % maximum length limit in pixel
    ...
    if  ~isempty(tiploc)
        ...
        SR_seg.tip(cnt)=SR_tip; %% modified
    ...
    %-------- calculate Intensity, RIS, SR' from tips of MV
    if  ~isempty(tiploc)
        max_radius=Rlim*Exfactor/pixelsize;
        radiushistedge=0:1:max_radius; %% pixel
...
if nframe>1
    ...
    v = VideoWriter([savefilename,'_RIS'],'Motion JPEG AVI'); %lossless compression
    v.FrameRate=2;
    v.Quality=100;
    open(v)
    writeVideo(v,mov)
...
p1=errorbar(histedgelengthnm(1:end-1),statresult.comb_distance_tip_ch1.median,statresult.comb_distance_tip_ch1.ste,'Marker','o');%,'MarkerEdgeColor','r')
...
subplot(1,4,2);
p1=errorbar(histedgelengthnm(1:end-1),statresult.comb_distance_tip_ch2.median,statresult.comb_distance_tip_ch2.ste,'Marker','o');%,'MarkerEdgeColor','r')
xlim([0,histedgelengthnm(end)])
xlabel('distance from tips (nm)')
ylabel(['Intensity'])
...
%----  <subplot1: RIS plot > ----%
subplot(1,4,3);
p1=errorbar(histedgelengthnm(1:end-1),statresult.comb_RIS_from_MVtip.median,statresult.comb_RIS_from_MVtip.ste,'Marker','o');%,'MarkerEdgeColor','r')
hline = refline([0 0]);
...
%----  <subplot4: SR' plot > ----%
subplot(1,4,4);
p1=errorbar(histedgelengthnm(1:end-1),statresult.comb_SR_from_MVtip.mean,statresult.comb_SR_from_MVtip.ste,'Marker','o');%,'MarkerEdgeColor','r')
hline = refline([0 0]);
hline.Color=[0.5,0.5,0.5];
xlabel('distance from tips (nm)')
ylabel('SR''')
